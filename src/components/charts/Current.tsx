...
export function Current() {
    ...
    useEffect(() => {
        async function fetchStats() {
            ...
            if (currHour.length > 0) {
                const avg = currHour.reduce((s, r) => s + r.current, 0) / currHour.length;
                const max = Math.max(...currHour.map(r => r.current));

                const prevAvg = prevHour.length > 0
                    ? prevHour.reduce((s, r) => s + r.current, 0) / prevHour.length
    ...
    return (
       ...
       >
        <Reading measurement="Generator Current" measureValue={latest ? latest.current.toFixed(2) : '-'} measureUnit="A" />
        <Reading measurement="Average Current" measureValue={stats.avg.toFixed(2)} measureUnit="A" percentChange={stats.pctChange} />
