...
Cross-Platform GUI (macOS & Windows Compatible) - Zero Emojis, High-Contrast UI
"""
...
class CustomButton(tk.Frame):
    ...
    Avoids native OS button styling glitches.
    """
...
class VideoFrameNavigatorApp:
    def __init__(self, root):
        ...
        # Auto-detect default paths if present in parent directories
        for v_cand in ["multimeter_extractor/input_videos/Voltage Readings .mp4", "../multimeter_extractor/input_videos/Voltage Readings .mp4", "Voltage_Readings.mp4"]:
            if os.path.exists(v_cand):
                self.voltage_path.set(os.path.abspath(v_cand))
                break
        for c_cand in ["multimeter_extractor/input_videos/Current readings.mp4", "../multimeter_extractor/input_videos/Current readings.mp4", "Current_Readings.mp4"]:
            if os.path.exists(c_cand):
                self.current_path.set(os.path.abspath(c_cand))
                break
    ...
    def _load_videos(self):
        ...
        step = 1.0 / self.fps_mode.get() # 1.0s or 0.5s
