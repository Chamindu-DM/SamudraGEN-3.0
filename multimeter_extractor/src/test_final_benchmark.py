...
def extract_perfect_glass(frame):
    h, w = frame.shape[:2]
    ...
    # Exact glass region in casing
    glass = casing[int(mh*0.10):int(mh*0.30), int(mw*0.19):int(mw*0.81)]
    glass = cv2.resize(glass, (400, 180))
    return glass

def decode_glass(glass):
...
print("\n" + "="*65)
print(" 🎯 FINAL ACCURACY BENCHMARK ON HUMAN-EYE GROUND TRUTH (120s..133s) ")
print("="*65)
...
for t_sec, true_val in GROUND_TRUTH.items():
    ...
    if not ret:
        continue
    glass = extract_perfect_glass(frame)
...
cap.release()
print("-" * 65)
print(f"Benchmark Accuracy: {correct}/{len(GROUND_TRUTH)} ({correct/len(GROUND_TRUTH)*100:.1f}%)")
