...
def extract_padded_sift_lcd(frame, ref_frame, ref_corners):
    ...
    if len(good) >= 6:
        ...
        if H is not None:
            ...
            # Slightly expand right and bottom by 4% to ensure last digit and dot are fully captured
            center = np.mean(cur_corners, axis=0)
            expanded = center + (cur_corners - center) * 1.05
...
# Use t=116s as reference anchor
cap.set(cv2.CAP_PROP_POS_FRAMES, int(116 * fps))
_, ref_frame = cap.read()
ref_corners = ann["t_0116s"]["corners"]

print("\n" + "="*65)
print(" 🎯 EXPANDED SIFT HOMOGRAPHY BENCHMARK (120s..133s) ")
print("="*65)
...
for t_sec, true_val in GROUND_TRUTH.items():
    ...
    if not ret:
        continue
    glass, _ = extract_padded_sift_lcd(frame, ref_frame, ref_corners)
    if glass is not None:
        cv2.imwrite(f"multimeter_extractor/debug_frames/padded_sift_t{t_sec}.jpg", glass)
