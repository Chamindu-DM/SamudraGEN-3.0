...
def extract_casing_and_lcd(frame):
    h, w = frame.shape[:2]
    ...
    # Crop LCD window (y: 11% to 35%, x: 18% to 82%)
    lcd_crop = mm_upright[int(mh*0.11):int(mh*0.35), int(mw*0.18):int(mw*0.82)]
    lcd_crop = cv2.resize(lcd_crop, (400, 180))
    return lcd_crop, src_orig
...
print("\n" + "="*60)
print(" 🎯 CASING-ALIGNED BENCHMARK TEST (120s..133s) ")
print("="*60)
...
for t_sec, true_val in GROUND_TRUTH.items():
    ...
    if not ret:
        continue
    lcd, _ = extract_casing_and_lcd(frame)
    if lcd is not None:
        cv2.imwrite(f"multimeter_extractor/debug_frames/bench_lcd_t{t_sec}.jpg", lcd)
        pred_val = decode_lcd(lcd)
